...
(function($, window, document) {
   ...
   var EmojiArea_WYSIWYG = function($textarea, id, options) {
        ...
        if (!this.options.norealTime) {
            changeEvents += ' keyup keydown keypress';
        }
...
//      scrollHeight -= parseInt(this.css('padding-top')) + parseInt(this.css('padding-bottom'));
    ...
    EmojiMenu.prototype.reposition = function() {
        ...
        if (!this.tether) {
            var offset = "0 12px";
            console.log("this.options.popUpOffSet",this.emojiarea.options.popUpOffSet)
            if(this.emojiarea.options.popUpOffSet){
                offset = this.emojiarea.options.popUpOffSet;
            }
